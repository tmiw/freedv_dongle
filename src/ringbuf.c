...
size_t
ringbuf_buffer_size(const struct ringbuf_priv_t *rb)
{
    return rb->size;
}
...
size_t
ringbuf_capacity(const struct ringbuf_priv_t *rb)
{
...
ringbuf_end(const struct ringbuf_priv_t *rb)
{
    return rb->buf + ringbuf_buffer_size(rb);
}

size_t
ringbuf_bytes_free(const struct ringbuf_priv_t *rb)
{
    if (rb->head >= rb->tail)
...
size_t
ringbuf_bytes_used(const struct ringbuf_priv_t *rb)
{
    return ringbuf_capacity(rb) - ringbuf_bytes_free(rb);
}

int
ringbuf_is_full(const struct ringbuf_priv_t *rb)
{
    return ringbuf_bytes_free(rb) == 0;
}

int
ringbuf_is_empty(const struct ringbuf_priv_t *rb)
{
    return ringbuf_bytes_free(rb) == ringbuf_capacity(rb);
}

const void *
ringbuf_tail(const struct ringbuf_priv_t *rb)
{
    return rb->tail;
}

const void *
ringbuf_head(const struct ringbuf_priv_t *rb)
{
    return rb->head;
}
...
size_t
ringbuf_findchr(const struct ringbuf_priv_t *rb, int c, size_t offset)
{
