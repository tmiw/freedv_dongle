...
int main(int argc, char** argv)
{
    ...
    while(1)
    {
        ...
        for (size_t i = 0; i < encoded_audio_size; i += sizeof(short)*DONGLE_AUDIO_LENGTH)
        {
            ...
            ptr += sizeof(short)*DONGLE_AUDIO_LENGTH;
        }
        
        // Force Teensy out of sync
        ptr = random_data;
        for (size_t i = 0; i < sizeof(random_data); i += sizeof(short)*DONGLE_AUDIO_LENGTH)
        {
            send_audio_packet(serialPort, ptr, 0);
